...
export default async function handler(req, res) {
  ...
  // req.query.path berisi array segmen setelah /api/anime/animasu/
  const { path = [] } = req.query;
  const subPath = Array.isArray(path) ? path.join("/") : String(path || "");

  // Susun ulang query string asli, buang parameter "path" bawaan Vercel
  ...
  try {
    const upstreamRes = await fetch(targetUrl, {
      ...
      // Jangan tunggu upstream selamanya
    ...
    // Cache di edge Vercel 5 menit, boleh sajikan versi lama sampai 1 jam
    // sambil diperbarui di background (mengurangi beban ke API asli).
